...
download_track() {
    ...
    if ! rclone --config "$RCLONE_CONF" copy "$path" "$DOWNLOADS_DIR/" -v --stats 5s 2>&1 | tee "$INFOFIFO"; then
...
play_track() {
    ...
    local format=$(ffprobe -v error -select_streams a:0 -show_entries stream=codec_name -of default=noprint_wrappers=1:nokey=1 "$fullname" 2>/dev/null)
    ...
    case "$format" in
        opus)
            if ! opusdec --rate 48000 --force-stereo "$fullname" - 2>"$INFOFIFO" > "$PCMFIFO"; then
            ...
            ;;
        mp3)
            if ! mpg123 --rate 48000 --encoding s16 --stereo -s "$fullname" 2>"$INFOFIFO" > "$PCMFIFO"; then
            ...
            ;;
        *)
            log_message "Unsupported format: $format (converting with ffmpeg)"
            if ! ffmpeg -nostdin -hide_banner -loglevel error -i "$fullname" -f s16le -ar 48000 -ac 2 - 2>"$INFOFIFO" > "$PCMFIFO"; then
